...
# FIXED OS DOWNLOAD - Solusi untuk download OS image yang reliable
# ======================================

echo "Windows 2019 akan diinstall"

# PILIHOS dengan direct link yang reliable (ganti dengan link yang valid)
# Contoh link alternatif - sesuaikan dengan source Windows 2019 yang Anda miliki
PILIHOS="https://archive.org/download/windows-2019-server/Windows%202019%20Server.gz"
# Alternatif lain jika link di atas tidak work:
# PILIHOS="https://example-server.com/windows2019.gz"
...
# Function untuk download OS dengan multiple methods
download_os() {
    local url="$1"
    local output="$2"
    echo "Mengunduh OS image dari: $url"
    
    # Method 1: Menggunakan wget dengan retry
    echo "Method 1: Menggunakan wget..."
    if wget --no-check-certificate --tries=3 --timeout=60 -O "$output" "$url"; then
        echo "✓ Download berhasil dengan wget"
        return 0
    fi
    
    # Method 2: Menggunakan curl dengan retry
    echo "Method 2: Menggunakan curl..."
    if curl -L -k --retry 3 --connect-timeout 60 -o "$output" "$url"; then
        echo "✓ Download berhasil dengan curl"
        return 0
    fi
    
    # Method 3: Menggunakan aria2c jika tersedia
    if command -v aria2c &> /dev/null; then
        echo "Method 3: Menggunakan aria2c..."
        if aria2c --check-certificate=false --timeout=60 -o "$output" "$url"; then
            echo "✓ Download berhasil dengan aria2c"
            return 0
        fi
    fi
...
# Download dan install OS dengan method yang lebih reliable
echo "Mengunduh dan menginstall Windows 2019..."
OS_FILE="/tmp/windows2019.gz"

# Download OS image
if download_os "$PILIHOS" "$OS_FILE"; then
    echo "✓ Download OS berhasil"
    ...
    echo "✗ Download OS gagal, menggunakan metode alternatif..."
    
    # Metode alternatif - streaming download langsung
    echo "Mencoba metode streaming download..."
    if wget --no-check-certificate -O- "$PILIHOS" | gunzip | dd of=/dev/vda bs=3M status=progress; then
        echo "✓ Metode streaming berhasil"
    else
        echo "✗ Semua metode download OS gagal"
        echo "Silakan cek link PILIHOS atau koneksi internet"
        exit 1
    fi
fi
...
echo "========================================"
echo "Server akan restart dalam 3 detik..."
