...
:: HAPUS FILE DPART.BAT DENGAN KOMBINASI ATTRIB + SCHEDULED TASK
echo [INFO] Menghapus dpart.bat dari Startup...
set "startup_path=%ProgramData%\Microsoft\Windows\Start Menu\Programs\Startup\dpart.bat"

:: Method 1: Clear attributes dan hapus normal
attrib -r -h -s "!startup_path!" >nul 2>&1
timeout 1 >nul
del /f /q "!startup_path!" >nul 2>&1

:: Method 2: Jika masih ada, buat scheduled task untuk menghapus
if exist "!startup_path!" (
    echo [INFO] Membuat scheduled task untuk penghapusan...
    echo @ECHO OFF > "%TEMP%\final_cleanup.bat"
    echo timeout 5 >> "%TEMP%\final_cleanup.bat"
    echo del /f /q "!startup_path!" >> "%TEMP%\final_cleanup.bat"
    echo del /f /q "%%TEMP%%\final_cleanup.bat" >> "%TEMP%\final_cleanup.bat"
    start /min "" "%TEMP%\final_cleanup.bat"
)

:: RESTART SISTEM - PASTIKAN INI SELALU DIJALANKAN
echo.
echo ========================================
