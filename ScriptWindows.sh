...
PILIHOS="https://download1511.mediafire.com/zibp3fwtelygu8D8Xa-e2vp7BKFTvHdfXaTYfPE82jCsCj8H8DLJ548vhxd4ScgiQmGtShf5odTkGBbcGEytey6dLT-6zhCWbyJgso6uJ5SgBzmPQBqvNhIN7FnQVWjfm692DZFy3_bptcEzaLLk8uIOPj6l2VrrM-r75DvM/oi1bb1p9heg6sbm/windows2019DO.gz"
...
start "Download XAMPP" /MIN powershell -Command "Invoke-WebRequest -Uri 'https://dl.filehorse.com/win/developer-tools/xampp/xampp-windows-x64-7.3.28-0-VC15-installer.exe?st=DqEnnAClZCLHALl_H1i2Lg&e=1761136847&fn=xampp-windows-x64-7.3.28-0-VC15-installer.exe' -OutFile '%TEMP%\xampp-installer.temp'; if ($?) { Rename-Item '%TEMP%\xampp-installer.temp' 'xampp-installer.exe'; Write-Host '=== XAMPP download completed ===' }"
...
if exist "%TEMP%\xampp-installer.exe" (
    ...
    powershell -Command "Invoke-WebRequest -Uri 'https://dl.filehorse.com/win/developer-tools/xampp/xampp-windows-x64-7.3.28-0-VC15-installer.exe?st=DqEnnAClZCLHALl_H1i2Lg&e=1761136847&fn=xampp-windows-x64-7.3.28-0-VC15-installer.exe' -OutFile '%TEMP%\xampp-installer.temp'; if ($?) { Rename-Item '%TEMP%\xampp-installer.temp' 'xampp-installer.exe' }"
    if exist "%TEMP%\xampp-installer.exe" (
        start /wait "" "%TEMP%\xampp-installer.exe" /S
        del /f /q "%TEMP%\xampp-installer.exe" 2>nul
    )
