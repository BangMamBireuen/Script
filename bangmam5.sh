...
echo Mengunduh semua aplikasi secara bersamaan...
powershell -Command "
\$jobs = @()
\$jobs += Start-Job -ScriptBlock { Invoke-WebRequest -Uri 'https://dl.google.com/chrome/install/latest/chrome_installer.exe' -OutFile \"\$env:TEMP\ChromeInstaller.exe\" }
\$jobs += Start-Job -ScriptBlock { Invoke-WebRequest -Uri 'https://dl.google.com/drive-file-stream/GoogleDriveSetup.exe' -OutFile \"\$env:TEMP\GoogleDriveSetup.exe\" }
\$jobs += Start-Job -ScriptBlock { Invoke-WebRequest -Uri 'https://github.com/PostgresApp/PostgresApp/releases/download/v2.3.5/Postgres-2.3.5-10-11-12.dmg' -OutFile \"\$env:TEMP\postgresql-9.4.26.1.exe\" }
\$jobs += Start-Job -ScriptBlock { 
    \$url1 = 'https://sourceforge.net/projects/xampp/files/XAMPP%20Windows/7.4.29/xampp-windows-x64-7.4.29-0-VC15-installer.exe/download'
    \$url2 = 'https://downloads.sourceforge.net/project/xampp/XAMPP%20Windows/7.4.29/xampp-windows-x64-7.4.29-0-VC15-installer.exe'
    try {
        Invoke-WebRequest -Uri \$url1 -OutFile \"\$env:TEMP\xampp-installer.exe\"
    } catch {
        Invoke-WebRequest -Uri \$url2 -OutFile \"\$env:TEMP\xampp-installer.exe\"
    }
}
\$jobs += Start-Job -ScriptBlock { Invoke-WebRequest -Uri 'https://github.com/notepad-plus-plus/notepad-plus-plus/releases/download/v7.8.5/npp.7.8.5.Installer.x64.exe' -OutFile \"\$env:TEMP\notepadplusplus-installer.exe\" }
\$jobs += Start-Job -ScriptBlock { Invoke-WebRequest -Uri 'https://www.win-rar.com/fileadmin/winrar-versions/winrar/winrar-x64-713.exe' -OutFile \"\$env:TEMP\winrar-installer.exe\" }
...
echo Semua download selesai! Memulai instalasi...

:: Install aplikasi satu per satu
...
if exist "%TEMP%\ChromeInstaller.exe" (
    ...
    echo Gagal menemukan Chrome installer
)
...
if exist "%TEMP%\GoogleDriveSetup.exe" (
    ...
    echo Gagal menemukan Google Drive installer
)
...
if exist "%TEMP%\postgresql-9.4.26.1.exe" (
    ...
    echo Gagal menemukan PostgreSQL installer
)
...
if exist "%TEMP%\notepadplusplus-installer.exe" (
    ...
    echo Gagal menemukan Notepad++ installer
)
...
if exist "%TEMP%\winrar-installer.exe" (
    ...
    echo Gagal menemukan WinRAR installer
)
